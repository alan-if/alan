...
THE floor ISA room_object
	...
	IS NOT movable.
	CONTAINER 			
		-- to allow 'empty/pour something on floor'
	DESCRIPTION ""
...
-- gives a better impression than if you just use the default message. 
...
-- called e.g. 'Darkness' and wish to make it lit at some point in the game, the name will still be  
-- 'Darkness' even if the location description can be changed to describe the illuminated location. 
...
-- 	VERB switch_on
-- 		DOES ONLY
-- 			FOR EACH d1 ISA dark_location 
--				DO
--					FOR EACH ls ISA LIGHTSOURCE, AT d1						
--						MAKE ls lit.
--					END FOR.
--				MAKE d1 lit.	
-- 					-- This line is not necessarily needed because the lit lightsource
--					-- in each dark location already makes the dar location lit.
--			END EACH.			  
-- 	END VERB.
-- END THE.

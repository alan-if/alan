...
{
    for (List *lst = stps; lst != NULL; lst = lst->next) {
...
Aaddr generateSteps(List *stps)
{
    ...
    for (List *l = stps; l != NULL; l = l->next) {
        Step *step = l->member.stp;
    ...
    adr = nextEmitAddress();
    for (List *l = stps; l != NULL; l = l->next) {
        stepEntry.after = l->member.stp->afteradr;
        stepEntry.exp = l->member.stp->expadr;
        stepEntry.stms = l->member.stp->stmadr;

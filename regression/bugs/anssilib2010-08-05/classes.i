...
-- instance 'hero'(=the player character). Many of these subclasses are frequently 
...
--
-- The following subclasses for actors are defined in this library:


-- NAMED_ACTOR
	-- has no article in front of the name when mentioned
	-- e.g. "You see Spot here."
	-- is not able to talk; this subclass is useful e.g. when defining animals
...
-- Here is a quick overview for using the subclass 'clothing'. A more closely-detailed
-- description of this subclass is found below this overview.


-- A piece of clothing in your game should look something similar to these four examples:
...
-- The quick guide ends here. The clothing table follows right after. The text following the 
-- table gives more details about the principles and the use of this subclass. 
...
-- We define the 'switch' verb here (a mere 'switch' with no 'on' or 'off'
-- following). This verb switches a device off if the device is on, and vice versa.
...
EVERY liquid ISA OBJECT
	CONTAINER 		-- we declare this class a container to make it work with the
				-- verb 'pour'(see 'verbs.i'), and also to make possible commands 
				-- such as 'take pearl from water', 'throw sack into water' 
				-- and 'look into water'.

	VERB take
		DOES ONLY "You cannot carry" SAY THE THIS. "around in your bare hands."
		-- a liquid usually needs to be put into a container before it can be 
		-- carried around
	END VERB.
...
-- and list there manually all relevant containers that the hero might carry,
-- thus:
--
-- VERB inventory
-- 	DOES LIST hero.
...
-- This is to avoid numerous new subclasses such as female_actor, named_female_actor,
...
THE hero ISA ACTOR
	...
	IS NOT lying_down.
  	...
  	LIMITS	-- remove this whole section (from this line until 'HEADER') if you wish to have no limits to
...
-- Please refer also 'verbs.i' where there are numerous verb checks for the hero, so that the hero cannot e.g. kick himself etc.

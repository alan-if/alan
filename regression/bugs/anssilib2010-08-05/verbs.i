...
----- This library file defines common verbs needed in gameplay. The verbs are listed alphabetically.
...
----- for the player to type commands such as both "put ball in box" and "put ball into box", etc:
  ...
  VERB ask
    WHEN act
      CHECK act HAS can_talk
        	ELSE "That's not something you can talk to."
	AND act IS reachable			-- in some situations, you might wish to remove this check,
							-- e.g. if the non-player character is speaking on the phone with the hero
...
SYNONYMS beat, fight, hit, kick, punch = attack.
...
SYNTAX break_with = break (obj1) 'with' (obj2)
		WHERE obj1 ISA OBJECT
			ELSE "That's not something you can break."
		AND obj2 ISA OBJECT
			ELSE "That's not something you can break things with."
...
ADD TO EVERY OBJECT
   VERB break_with
	...
	AND CURRENT LOCATION IS lit
		ELSE "It is too dark to see."
	DOES		
		"You can't break" SAY THE obj1. "with" SAY THE obj2. "."
   END VERB.
END ADD TO.
...
-- The verbs 'empty' and 'pour' work a bit differently from each other.
-- You can both empty and pour a container object, and its contents will be 
-- emptied in (on the floor or on the ground of) the current location if you don't 
-- specify anything else.
-- However, as far as liquids are concerned, you can only pour liquids
-- but not empty them - it is only possible to empty container objects.
-- This entails that if there is for example a bottle in your game that contains some water, both
-- 
-- 'pour bottle' AND 'pour water' have the same effect in the game,
--
-- whereas it is only possible to say
--
-- 'empty bottle' but NOT 'empty water'.
--
-- If you wish to change this, tweak these two verbs as you see fit for your purposes
-- (for example, delete one of the verbs altogether and make it a synonym of the other one).
...
ADD TO EVERY OBJECT
  VERB 'empty'
	...
	AND COUNT ISA OBJECT, IN obj > 0
		ELSE "There is nothing in" SAY THE obj. "."
	DOES 
	   	IF obj ISA LIQUID
			THEN "That's not something you can empty."
		ELSIF obj NOT IN hero
			THEN
				IF obj IS takeable 
					THEN  "(taking" SAY THE obj. "first)$n"
						LOCATE obj AT hero.
						"You empty the contents of" SAY THE obj.
							IF floor HERE
								THEN "on the floor"
								ELSE "on the ground"
							END IF.
					...
					ELSE "You don't have" SAY THE obj. "."
				END IF.
			ELSE 	"You empty the contents of" SAY THE obj.
					IF floor HERE
						THEN "on the floor"
						ELSE "on the ground"
					END IF.
...
SYNTAX empty_in = 'empty' (obj1) 'in' (obj2)
		...
		AND obj1 ISA CONTAINER 		-- only a container object can be emptied into something
			ELSE "That's not something you can pour."
		AND obj2 ISA OBJECT
			ELSE "Pouring" SAY THE obj1. "into" SAY THE obj2. "is not possible."
...
ADD TO EVERY OBJECT
  VERB empty_in
	...
	CHECK CURRENT LOCATION IS lit
		ELSE "It is too dark to see."
      AND COUNT ISA OBJECT, IN obj1 > 0
		ELSE SAY THE obj1. "is already empty."	
	AND obj1 NOT IN obj2
		ELSE SAY THE obj1. "is already in" SAY THE obj2. "."
	AND obj1 <> obj2
		ELSE "It doesn't make sense to empty something into itself."
	AND obj1 IS reachable
		ELSE SAY THE obj1. "is out of your reach."
	...
	DOES 
		IF obj1 ISA LIQUID
			THEN "That's not something you can empty."
	      ELSIF obj1 NOT IN hero
			THEN
				IF obj1 IS takeable
					THEN  "(taking" SAY THE obj1. "first)$n"
						LOCATE obj1 IN hero.
						"It would serve no purpose to empty" SAY THE obj1. "into" SAY THE obj2. "."
						-- If you need to make this work, use the following lines instead:
						-- "You empty" SAY THE obj1. "into" SAY THE obj2. "."
						-- EMPTY obj1 IN obj2.
					ELSE "You don't have" SAY THE obj1. "."
				END IF.
			ELSE "It would serve no purpose to empty" SAY THE obj1. "into" SAY THE obj2. "."
				-- If you need to make this work, use the following lines instead:
				-- "You empty" SAY THE obj1. "into" SAY THE obj2. "."
				-- EMPTY obj1 IN obj2.
		END IF. 
...
SYNTAX empty_on = 'empty' (obj1) 'on' (obj2)
		WHERE obj1 ISA OBJECT
			ELSE "That's not something you can pour."
		AND obj1 ISA CONTAINER 		-- only (the contents of) a container object can be poured in something
			ELSE "That's not something you can pour."
		AND obj2 ISA THING
		...
		IF obj1 NOT IN hero
			THEN
				IF obj1 IS takeable
					THEN  "(taking" SAY THE obj1. "first)$n"
						LOCATE obj1 IN hero.
						"It would serve no purpose to empty" SAY THE obj1. "on" SAY THE obj2. "."
						-- (or, to make it work, use the following lines instead of the above:
						-- "You empty" SAY THE obj1. "on" SAY THE obj2. ". Nothing special happens."
						-- 	EMPTY obj1 AT hero.)	
					ELSE  "You don't have" SAY THE obj1. "."
				END IF.
			ELSE "It would serve no purpose to pour" SAY THE obj1. "on" SAY THE obj2. "."
				-- (or, to make it work, use the following lines instead of the above:
				-- "You pour" SAY THE obj1. "on" SAY THE obj2. ". Nothing special happens."
				-- EMPTY obj1 AT hero.)
		END IF.
END VERB.
END ADD TO.
...
ADD TO EVERY OBJECT
  VERB fill_with
	...
	AND CURRENT LOCATION IS lit
		ELSE "It is too dark to see."
	DOES 
		"You fill" SAY THE obj1. "with" SAY THE obj2. "."
		LOCATE obj2 IN obj1.
  END VERB.
END ADD TO.
...
-- another formulation added, to guide players to use the right syntax:
...
VERB fire_at_error
	DOES 
		"Please use the formulation FIRE WEAPON AT OBJECT to fire at something."
END VERB.
END ADD TO.
...
SYNONYMS walk = go.		-- here we define a synonym for the predefined parser word 'go'
					...
					-- (as well as e.g. 'go east' and 'walk east').
...
VERB i
	DOES 
		...
		-- If you live the above addition out, the outcome will be just "You are carrying a bag.", with
  ...
  VERB kiss
	...
	DOES
		IF obj ISA ACTOR
			...
			ELSE "Nothing much would be achieved by that."
		END IF.
  END VERB.
END ADD TO.
...
VERB knock_error
	DOES 
		"Please use the formulation KNOCK ON OBJECT to knock on something."
END VERB.
...
SYNTAX light = light (obj)
		WHERE obj ISA LIGHTSOURCE
			ELSE "That's not something you can lit."
  ...
  VERB open
    ...
    DOES
		...
		"You open" SAY THE obj. 
			IF obj ISA LISTABLE_CONTAINER
				THEN "and look into it"
			END IF.
		"."
  END VERB.
END ADD TO.
...
ADD TO EVERY OBJECT
  VERB play_with
	CHECK CURRENT LOCATION IS lit
		ELSE "It is too dark to see."
	DOES 
		"After second thoughts you don't find it purposeful to start playing with" SAY THE obj. "."
  END VERB.
END ADD TO.
...
-- The verbs 'pour' and 'empty' work a bit differently from each other.
-- You can both pour and empty a container object and its contents will be 
-- emptied in (on the floor or on the ground of) the current location if you don't 
-- specify anything else.
-- However, as far as liquids are concerned, you can only pour liquids
-- but not empty them - it is only possible to empty container objects.
...
ADD TO EVERY OBJECT
  VERB pour
	...
	AND CURRENT LOCATION IS lit
		ELSE "It is too dark to see."
	AND COUNT ISA OBJECT, IN obj > 0
		ELSE "There's nothing in" SAY THE obj. "."
	DOES 	
	      IF obj NOT IN hero
			THEN
				IF obj IS takeable
...
SYNTAX pour_in = pour (obj1) 'in' (obj2)
		WHERE obj1 ISA OBJECT
			ELSE "That's not something you can pour."
		AND obj1 ISA CONTAINER 		-- only (the contents of) a container object can be poured into something
			ELSE "That's not something you can pour."
		AND obj2 ISA OBJECT
			ELSE "Pouring" SAY THE obj1. "into" SAY THE obj2. "is not possible."
		AND obj2 ISA CONTAINER
	      ...
	      IF obj1 NOT IN hero
			THEN
				IF obj1 IS takeable
					THEN  "(taking" SAY THE obj1. "first)$n"
						LOCATE obj1 IN hero.
						"It would serve no purpose to pour" SAY THE obj1. "into" SAY THE obj2. "."
						-- If you need to make this work, use the following lines instead:
					...
					ELSE "You don't have" SAY THE obj1. "."
				END IF.
			ELSE "It would serve no purpose to pour" SAY THE obj1. "into" SAY THE obj2. "."
				-- If you need to make this work, use the following lines instead:
				-- "You pour" SAY THE obj1. "into" SAY THE obj2. "."
				-- IF obj1 ISA LIQUID
				--	THEN LOCATE obj1 IN obj2.
		...
		IF obj1 NOT IN hero
			THEN
				IF obj1 IS takeable
					THEN  "(taking" SAY THE obj1. "first)$n"
						LOCATE obj1 IN hero.
						"It would serve no purpose to pour" SAY THE obj1. "on" SAY THE obj2. "."
						-- (or, to make it work, use the following lines instead of the above:
						-- "You pour" SAY THE obj1. "on" SAY THE obj2. ". Nothing special happens."
						-- IF obj1 ISA LIQUID
						-- 	THEN LOCATE obj1 AT hero.
						-- 	ELSE EMPTY obj1 AT hero.
						-- END IF.	
					ELSE  "You don't have" SAY THE obj1. "."
				END IF.
			ELSE "It would serve no purpose to pour" SAY THE obj1. "on" SAY THE obj2. "."
		...
		WHERE obj ISA OBJECT
			ELSE 
				IF obj ISA ACTOR
					THEN 
						IF obj = hero
							THEN "It doesn't make sense to shake yourself."
							ELSE SAY THE obj. "wouldn't probably appreciate that."
						END IF.
...
-- this verb is interpreted as 'incomplete', guiding the player to use the formulation
-- 'shoot THING with WEAPON'
...
ADD TO EVERY THING
  VERB shoot
	CHECK obj <> hero 
		ELSE "There's no need to be that desperate."
	DOES 
		"To shoot something, use the formulation SHOOT THING WITH WEAPON."
  END VERB.
END ADD TO.


-- another incomplete 'shoot' formulation added, to guide players to use the right phrasing:
...
SYNTAX shoot_with = shoot (obj1) 'with' (obj2)
	 ...
	 shoot_with = shoot (obj2) 'at' (obj1).
			-- to count for player input such as 'shoot rifle at bear'
...
ADD TO EVERY OBJECT
  VERB take
    ...
    AND obj IS takeable
      ELSE 
		IF obj ISA SCENERY
			THEN "You deem" SAY THE obj. "as not important for your purposes and leave it where it is."
			ELSE "That's not something you can take."
		END IF.
    AND obj IS movable
	...
	ELSE "It is too dark to see."
    DOES
	IF obj DIRECTLY IN hero						--| This distinction is made in order to 				
		THEN "You already have" SAY THE obj. "."  	--| cover cases where the hero wants to take
	ELSIF obj IN hero							--| something out of a container (s)he's carrying.
		THEN "Taken."						--| I.e. the 'directly' checks that the hero is not carrying
			LOCATE obj IN hero.				--| the object in her/his hands already; the first 'elsif'  
										--| here allows e.g. a wallet to be taken out of a jacket 
										--| that the hero is carrying in her/his hands.
	ELSIF obj IN worn		-- i.e. the object to be taken is a piece of clothing that the player character is wearing;
...
ADD TO EVERY OBJECT
  VERB take_from
	...
	AND obj1 IS takeable
      	ELSE 
			IF obj1 ISA SCENERY
				THEN "You deem" SAY THE obj1. "as not important for your purposes and leave it where it is."
				ELSE "That's not something you can take."
			END IF.
	AND obj1 IS movable
				...
				IF obj1 IS takeable
					THEN  "(taking" SAY THE obj1. "first)$n"
						LOCATE obj1 IN hero.
						  IF obj2 IS inanimate
							THEN
	        						    "The" SAY obj1. "bounces harmlessly off"
			...
			ELSE
				IF obj2 IS inanimate
					THEN
		  				...
		  				LOCATE obj1 HERE.
					ELSE 
						SAY THE obj2. "catches" SAY THE obj1. "and tosses"
							IF THIS IS NOT plural
								THEN "it"
								ELSE "them"
							END IF.
		...
		IF obj1 NOT IN hero
			THEN 
				IF obj1 IS takeable
					...
					ELSE "You don't have" SAY THE obj1. "."
				END IF.
			ELSE "It's not possible to tie" SAY THE obj1. "to" SAY THE obj2. "."
		END IF.
  END VERB.
END ADD TO.
...
----- an ordinary object will default here to "That's not something you can turn on".


SYNTAX turn_on1 = turn 'on' (obj)
		WHERE obj ISA OBJECT
	    		ELSE "That's not something you can turn on."

    turn_on1 = switch 'on' (obj).
...
SYNTAX turn_off1 = turn off (obj)
		WHERE obj ISA OBJECT
	 	   	ELSE "You can't turn that off."

	turn_off1 = switch off (obj).
